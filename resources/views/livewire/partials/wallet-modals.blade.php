...
<x-modal wire:model="showReversalModal" title="Solicitar Estorno" on-close="closeReversalModal">
    <div class="space-y-4">
        <p class="text-sm text-slate-500 dark:text-slate-400">
            Você está solicitando o estorno de uma transferência. Por favor, detalhe o motivo para que a administração possa analisar o seu pedido.
        </p>

        <x-textarea-group 
            id="reversalReason" 
            model="reversalReason" 
            label="Motivo do Estorno" 
            placeholder="Ex: Pagamento duplicado, valor incorreto..." 
            rows="4" 
        />
    </div>

    <x-slot name="footer">
        <button type="button" wire:click="closeReversalModal" class="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg shadow-sm hover:bg-slate-50 transition-all dark:bg-slate-800 dark:text-slate-300 dark:border-slate-600 dark:hover:bg-slate-700">
            Cancelar
        </button>
        <button type="button" wire:click="requestReversal" wire:loading.attr="disabled" class="px-5 py-2 text-sm font-medium text-white bg-orange-600 border border-transparent rounded-lg shadow-sm hover:bg-orange-700 active:scale-95 transition-all flex items-center gap-2">
            <span wire:loading wire:target="requestReversal">
                <svg class="animate-spin h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
            </span>
            Enviar Solicitação
        </button>
